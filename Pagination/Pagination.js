...
const Card = styled.div`
  ...
  line-height: 37px;
  margin: 0 4px;
  background-color: ${props => (props.disabled ? '#eee' : props.active ? props.theme.color.primary : '#fff')};
      ...
      <Container style={style}>
        <PaginationContainer style={paginationContainerStyle}>
          ...
          {Array(displayItem > total ? total : displayItem)
            ...
            .map((v, i) => {
              ...
              return (
                <Card key={i} active={currentPage === page} size={size} onClick={() => this.handleChangePage(page)}>
                  {page}
                </Card>
              );
            })}
        ...
        <JumpPage
          ...
          onConfirm={pageValue => {
            ...
            if (/^[1-9][0-9]*$/.test(pageValue) && numberValue <= total && numberValue > 0) {
